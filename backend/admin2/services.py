...
def sync_attendance_xp(attendance):
    ...
    # --------------------------------------------------------
    ...
    elif attendance.attendance_source == "PLAYING_DAY":
        ...
        if attendance.status == "PRESENT":
            ...
            source = "PLAYING_DAY_PRESENT"



            reason = (

                "Volunteer was marked present on Playing Day."
...
def send_playing_day_emails(daily_status):

    """Send the Playing Day notification to all active registered volunteers."""

    from teachers.models import Teacher



    if daily_status.status != "PLAYING_DAY":

        return {"sent": 0, "failed": 0}



    volunteers = Teacher.objects.exclude(

        auth_user__is_active=False
    ...
    # Also exclude volunteers explicitly marked REMOVED.

    removed_ids = VolunteerAccountStatus.objects.filter(
    ...
    volunteers = volunteers.exclude(id__in=removed_ids)
    ...
    subject = "Jaago Team - Playing Day Notification"

    body = (

        f"Hello {{name}},\n\n"

        f"There will be a Playing Day on {{date}}.\n\n"

        "Please note that there will be no regular class assignment for this day.\n\n"
    ...
    for volunteer in volunteers:
        ...
        try:
            ...
            errors.append(
                ...
                f"{volunteer.user_id}: {exc}"

            )



    daily_status.playing_day_email_status = (

        "SENT" if failed == 0 else "FAILED"

    )

    daily_status.playing_day_email_sent_at = (

        timezone.now() if sent > 0 else None
